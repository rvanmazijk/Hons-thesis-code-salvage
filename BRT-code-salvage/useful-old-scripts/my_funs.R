...
# TODO: Are these needed now that I use `here::here()` for most paths?
datwd <- paste0(getwd(), "/Data/")
reswd <- paste0(getwd(), "/Results/")
# These two are still necessary though!
...
quant_plot <- function(x, y, dat,
    ...
    mq25 <- rq(formula = log(y) ~ log(x) + I(log(x) ^ 2),
               tau = 0.25,
               data = dat)
    y25 <- predict(mq25, list(x = x_seq))
    lines(x = x_seq, y = exp(y25),
          lty = 1, lwd = 1, col = "red")

    mq05 <- rq(formula = log(y) ~ log(x) + I(log(x) ^ 2),
               tau = 0.05,
               data = dat)
    y05 <- predict(mq05, list(x = x_seq))
    lines(x = x_seq, y = exp(y05),
          lty = 2, lwd = 1, col = "red")

...
#'           )
do_prelim_anal <- function(x = NULL,
                           agg_x = NULL,
                           rough_agg_x = NULL,
                           border,
                           border_buffer,
    ...
    # TODO: make pdf version less blurry? make tiff version bigger?
    ...
    if (save_grd) {
        dir.create(showWarnings = TRUE, paste0(
            reswd_temp,  # FIXME
            "raster_lists_",
            var_name, "_",
            region_name,
        ...
        for (i in seq_along(agg_x)) {
            writeRaster(agg_x[[i]], overwrite = TRUE, filename = paste0(
                reswd_temp,  # FIXME
                "raster_lists_",
                var_name, "_",
                region_name,
    ...
    if (plot_format == "PDF") {
        ...
        for (i in seq_along(agg_x)) {
            tiff(paste0(
                ...
                region_name,
                "_TIFFs/",
                "fact_", i, "_", format(Sys.time(), "%Y-%m-%d"), ".tif"  # FIXME?
            ))

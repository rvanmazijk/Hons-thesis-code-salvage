...
library(pacman)
p_load(
  magrittr, tidyverse, beepr,   reshape2,  here,   glue,
...
plot_fun_raster_loop <- function(x, f) {
  #if (!is.raster(x)) stop("x must be a raster")
  for (i in 1:length(f)) {
...
#' @example
#' plot_fun_raster(x = MAP_GCFR, f = sqrt)
...
#' @param x A raster
#'
#' @return Plots the modelled relationship (a simple \code{lm()}) between roughness values
#'         (as \code{raster::terrain(., "roughness")}) and absolute values of a raster
#'         \code{x}, with confidence & prediction intervals.
#'
...
#'         (as \code{raster::terrain(., "roughness")}) as a function of absolute
#'         values of a raster \code{x}.
...
#' custom_resample
#' Convenience fn to resample a raster to a diff resolution (bilinear is the default method)
#' @param x A raster
...
#' Same idea as `custom_resample_loop`, but with `aggregate()` instead, so as to only
#' create aligned, integer multiples of the original raster.
#' @param x A raster
#' @param facts A numeric vector, specifying the desired multiples
#'              of the \code{x}'s resolution
#' @param co_ord A CRS object's string (\code{std_CRS} is the default)
#' @param method A function name, specifing the resampling method (\code{mean} default)
...
custom_aggregate_loop <- function(x, facts, co_ord = std_CRS, method = mean) {
  ...
  for (i in 1:length(facts)) {
    custom_aggregate_list[[i]] <- raster::aggregate(x = x, fact = facts[i], fun = method)
  }
...
plot_from_list <- function(x) {
  if (!is.list(x)) stop("x must be a list")
  for (i in 1:length(x)) plot(x[[i]], main = names(x[i]))
}
...
plot_raster_list2 <- function(r, b) {
  if (!is.list(r)) stop("x must be a list")
  for (i in 1:length(r)) {
...
#' for some data-set of `x` vs `y`. The fn also has nice convenient label arguments.
#'
...
#' many_HASRs
#' Applies `shuffle_raster()`, then agg(), and then rough() to a raster however many
#' times you want, saving each list of rasters into a list (s.t. `out` = a list of lists)
...
#' many_HSARs
#' Applies agg(), then `shuffle_raster()`, and then rough() to a raster however many
#' times you want, saving each list of rasters into a list (s.t. `out` = a list of lists)
#' @param x A raster
#' @param how_many A number
#' @param facts A numeric vector, the factors of aggregation
...
#' many_SARs
#' Applies agg() and then `shuffle_raster()` to a raster however many times you want,
...
#' and fits a quadratic curve using `lm()` for the rotation ~ variable, for both PC1
#' & PC2
#'
#' @param x A list of class "prcomp" (i.e. A PCA model output from \code{prcomp()})
#' @param facts A numeric vector, the factors of aggregation (which for my purposes
#'              is the variable in the PCA), usually 1:15
#'
#' @return Plots of rotation ~ variable for PC1 & PC2, side-by-side
plot_PC_loadings <- function(x, facts) {

  if ((class(x) != "prcomp") || (!is.list(x))) {
  ...
  plot(
    x$rotation[, 1],
    ylim = c(-max(abs(x$rotation[, 1])), max(abs(x$rotation[, 1]))),
  ...
  abline(h = 0, lty = 2)
  fit <- lm(x$rotation[, 1] ~ c(1, facts) + I(c(1, facts) ^ 2))
  ...
  plot(
    x$rotation[, 2],
    ylim = c(-max(abs(x$rotation[, 2])), max(abs(x$rotation[, 2]))),
  ...
  axis(1, labels = c(1, facts), at = c(1, facts))
  abline(h = 0, lty = 2)
  fit <- lm(x$rotation[, 2] ~ c(1, facts) + I(c(1, facts) ^ 2))
  summary(fit)
  newdat <- data.frame(fact = seq(
    1, (length(facts) + 1),
    length.out = (length(facts) + 1)
...
#'
#' @example > # x should look like this (colnames NB, dimensions are just for e.g.):
#'          > x
#'          # A tibble: 1,781 x 19
...
plot_PC_geospace <- function(x, pc1, pc2,
  ...
  arrange_PC_geospace_child <- function(x_child, border, region,
  ...
  if (!is.null(region_b) && !is.null(border_b)) {
    x_a <- x %>%
...
do_PCAandkmeans <- function(region_a_dir, region_a_name, border_a,
                            region_b_dir = NULL, region_b_name = NULL, border_b = NULL,
                            var_name, do_kmeans = FALSE) {
  ...
  if (!is.null(region_b_dir) && !is.null(region_b_name)) {
    all_regions <- rbind(
      import_region_dfs(
  ...
  plot(
    ...
    ylim = c(-max(abs(all_regions_pca$x[, 2])), max(abs(all_regions_pca$x[, 2]))),
    xlim = c(-max(abs(all_regions_pca$x[, 1])), max(abs(all_regions_pca$x[, 1])))
  )
  abline(h = 0, lty = 2)
  abline(v = 0, lty = 2)

  plot_PC_loadings(all_regions_pca, 2:4)  # FIXME: make `fact` variable
  ...
  if (!is.null(region_b_dir) && !is.null(region_b_name) && do_kmeans) {
...
if (FALSE) { #<DEPREC>
  ...
  do_myPCA_two_regions_old <- function(region_a_dir, region_a_name,
                                       region_b_dir, region_b_name,
                                       var_name) {
